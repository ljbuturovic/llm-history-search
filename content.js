// content.js
console.log('[llm-history-search extension] Content script loaded');
...
function collectText() {
  ...
  if (provider === 'unknown') {
    console.log('[llm-history-search extension] Skipping unknown provider page');
    return;
  }

  // Skip generic pages that aren't actual conversations
  if (provider === 'gemini' && (pathname === '/app' || pathname === '/app/')) {
    console.log('[llm-history-search extension] Skipping generic Gemini app page');
    return;
  }
  ...
  if (provider === 'chatgpt' && pathname.startsWith('/g/') && !pathname.includes('/c/')) {
    console.log('[llm-history-search extension] Skipping ChatGPT project overview page');
    return;
  }
  ...
  console.log('[llm-history-search extension] Capturing thread:', thread);
  queueThread(thread);
}
...
function flushThreads() {
  ...
  if (!runtime?.id) {
    ...
    console.log('[llm-history-search extension] Runtime unavailable, dropping threads');
    pendingThreads.length = 0;
    return;
  }

  const threadsToSend = pendingThreads.splice(0, pendingThreads.length);
  console.log('[llm-history-search extension] Flushing', threadsToSend.length, 'threads');
      ...
      try {
        runtime.sendMessage({ type: "CAPTURE", thread }, (response) => {
          if (runtime.lastError) {
            ...
            console.error('[llm-history-search extension] Error sending message (attempt ' + attempt + '):', error);
            ...
            if (attempt === 1 && error.includes('Receiving end does not exist')) {
              console.log('[llm-history-search extension] Retrying after service worker wake-up...');
              setTimeout(() => sendWithRetry(2), 100);
            }
          } else {
            console.log('[llm-history-search extension] Message sent successfully', response);
          }
        });
      } catch (error) {
        console.error('[llm-history-search extension] Exception sending message:', error);
      }
    };
    sendWithRetry();
  });
